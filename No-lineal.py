...
#%% Non-linear
    ...
    ####################### Hay que tener cuidado con cambiar  de este código (utilizamos PID 2)
    s = sp.Symbol('s')
    G = (((I+m*L**2)*(M+m)-m*L**2)*s**4+(I+m*L**2)*b*s**2)/(-m*L*s**2)
    Gc = (num[0]*s**2+num[1]*s+num[2])/s
    M = (K*G)/(1+K*Gc*G)

    numer = [25.0*0.107549377288609, 25.0*0.655903706911359, 25.0*1.0]
    denom = [0.0537746886443044,0.32795185345568,0.392450622711391,0.722245485746348, -1.0]
    sys = control.tf(numer,denom)
    sys_poles = sys.pole()
    sys_zeros = sys.zero()
    print(sys_poles)
    ...
    plt.figure()    
    t, out = control.step_response(sys)
    plt.plot(t,out)
    plt.title('Perturbation response with PID controller')
    plt.xlabel('time (s)')
    plt.ylabel('$\\theta$ (rad)')
    ...
    # Utilizando el PID del no lineal
    ...
    num =[-13.3333333333333*0.055875, 0, -13.3333333333333*0.00285, 0, 0]
    den = [1, 0, 0]
    G = control.tf(num, den)

    ################ only valid for this G transfer function
    numerator = control.tf(num,[1])
    denominator = control.tf(den,[1])
    ##############
    ...
    arg = np.angle(G(sd)*Gc(sd), deg=True)
    print(f'Argument criterion with Gc: {arg} degrees')
...
#%%
    ...
    '''
    T = K * yout_nonlineal[:,2] / Gc; 
   
   # Evolution of theta: non-linear system
    plt.figure()
    plt.plot(t1,T, label='non-linear')
    plt.title('$\\theta$ variation along time')
    plt.legend(loc='best', shadow=True, framealpha=1)
    plt.grid(alpha=0.3)
    plt.ylabel('$\\theta$ (rad)')
